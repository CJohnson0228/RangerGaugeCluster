...
// Engine
void VehicleState::setEngineTemp(qreal v) { if (m_engineTemp != v) { m_engineTemp = v; emit engineTempChanged(); } }

// Fuel — fuelLevel is the raw ECU value; fuelRemaining and fuelRange are computed from it
void VehicleState::setFuelLevel(qreal v) {
    if (m_fuelLevel != v) {
        m_fuelLevel = v;
        emit fuelLevelChanged();
        emit fuelRemainingChanged();  // computed from fuelLevel
        emit fuelRangeChanged();      // computed from fuelLevel
    }
}

void VehicleState::setFuelEconomyLive(qreal v) { if (m_fuelEconomyLive != v) { m_fuelEconomyLive = v; emit fuelEconomyLiveChanged(); } }

void VehicleState::setFuelEconomyAverage(qreal v) {
    if (m_fuelEconomyAverage != v) {
        m_fuelEconomyAverage = v;
        emit fuelEconomyAverageChanged();
        emit fuelRangeChanged();      // computed from fuelEconomyAverage
    }
}
...
void VehicleState::setCruiseControlActive(bool v) { if (m_cruiseControlActive != v) { m_cruiseControlActive = v; emit cruiseControlActiveChanged(); } }
void VehicleState::setHiBeamsActive(bool v) { if (m_hiBeamsActive != v) { m_hiBeamsActive = v; emit hiBeamsActiveChanged(); } }
void VehicleState::setLoBeamsActive(bool v) { if (m_loBeamsActive != v) { m_loBeamsActive = v; emit loBeamsActiveChanged(); } }